...
function writePair(document: PDFKit.PDFDocument, label: string, value: string, x: number, y: number, width = 220) {
...
function sectionTitle(document: PDFKit.PDFDocument, title: string, y: number) {
  document
    .moveTo(40, y)
    .lineTo(555, y)
    .strokeColor("#d4c4b8")
...
export function buildInvoicePdf(invoice: InvoicePdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    y += 38;
    const charges: Array<[string, unknown]> = [
