...
const DEFAULT_DAMAGE_POLICY = "Damage, loss, stains, missing pieces, or repair costs may be deducted from the refundable security deposit.";
const DEFAULT_CANCELLATION_POLICY = "Cancellation and refund decisions are handled by store policy and depend on booking status and pickup timing.";
function toNumber(value) {
    return Number(value ?? 0);
}
function parseOptionalDate(value, key) {
...
function getInvoicePaymentStatus(totalPayable, amountPaid) {
...
invoicesRouter.get("/", asyncHandler(async (_request, response) => {
...
invoicesRouter.post("/", asyncHandler(async (request, response) => {
    ...
    const totalPayable = Math.max(0, rentalFee + securityDeposit + dryCleaningCharge + alterationCharge + deliveryCharge + otherCharge - discountAmount);
...
invoicesRouter.get("/:id/pdf", asyncHandler(async (request, response) => {
