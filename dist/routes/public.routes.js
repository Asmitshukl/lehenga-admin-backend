...
publicRouter.post("/orders", asyncHandler(async (request, response) => {
    ...
    const paymentMethod = rawPaymentMethod === PaymentMethod.ONLINE
    ...
    await prisma.checkoutPaymentAttempt.create({
        data: {
            ...
            paymentMethod,
            paymentAmount,
            checkoutPayload: JSON.parse(JSON.stringify(body)),
    ...
    response.status(201).json({
        ...
        data: {
            ...
            razorpayOrder: {
                ...
                name: "Lehenga Rental",
                description: paymentMethod === PaymentMethod.PICKUP
...
publicRouter.post("/payments/razorpay/verify", asyncHandler(async (request, response) => {
    ...
    const preparedItems = await prepareCheckoutItems(items);
    const summary = summarizePreparedCheckout(preparedItems);
    const expectedAmount = attempt.paymentMethod === PaymentMethod.PICKUP ? summary.securityDeposit : summary.totalAmount;
    ...
    const amountPaid = attempt.paymentMethod === PaymentMethod.PICKUP ? summary.securityDeposit : summary.totalAmount;
    const amountDueAtPickup = attempt.paymentMethod === PaymentMethod.PICKUP ? summary.subtotalAmount : 0;
    ...
    await prisma.$transaction([
        prisma.rentalOrder.create({
            data: {
                ...
                amountPaid,
                amountDueAtPickup,
                depositRefundStatus: summary.securityDeposit > 0
